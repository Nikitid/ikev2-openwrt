...
var ru = {
	...
	'Emergency fallback': 'Аварийный резерв',
	'Use WAN-provided DNS': 'Использовать DNS от провайдера',
	'Use DNS servers supplied by the WAN provider only when the configured resolver group fails.': 'Использовать DNS, полученные от WAN-провайдера, только если основная группа резолверов не отвечает.',
	'This is an explicit unencrypted fallback and is not used for tunnel-routed destinations.': 'Это явно включаемый нешифрованный резерв; для направляемых в туннель доменов он не используется.',
